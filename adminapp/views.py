...
def index(request):
    if 'admin_email' in request.COOKIES and request.session.has_key('admin_email'):
        ...
        # Get ordercount
        cur.execute("SELECT COUNT(*) FROM tbl_order_master")
        row = cur.fetchone()
        order_count=row[0]
        ...
        context = {'user_count': user_count,'trainer_count':trainer_count,'order_count':order_count}
...
def Useraddprocess(request):
    if request.method == 'POST':
        ...
        try:
            ...
            f = open("static/upload/"+str(photo),'wb')
            for i in photo:
                f.write(i)
            f.close()
            id_proof = request.FILES['id_proof']
            fi = open("static/upload/"+str(id_proof.name), 'wb')
            for i in idproof:
                fi.write(i)
            fi.close()
        except:
...
def ViewReport(request):
    if request.method == 'POST':
        ...
        cur.execute("SELECT * FROM `membership_master` WHERE `Start_Date` BETWEEN '{}' AND '{}' AND `End_Date` BETWEEN '{}' AND '{}'".format(start_date, end_date, start_date, end_date))
        data = cur.fetchall()
        return render(request,'Admin/ViewReport.html', {'mydata': data})
