...
conn = mcdb.connect(host="localhost", user="root", passwd="rish", database='myproject1')
...
def registerprocess(request):
    if request.method == 'POST':
        ...
        f = open("static/upload/"+str(img), 'wb')
        
        for i in photo:
            f.write(i)
        f.close()
        id_proof = request.FILES['id_proof']
        f = open("static/upload/"+str(id_proof), 'wb')
        for i in id_proof:
            f.write(i)
        f.close()
        # except:
...
def paymentprocess(request,id):
    ...
    if request.method == 'POST':
        ...
        f = open("static/upload/"+str(recp), 'wb')
        for i in recp:
            f.write(i)
        f.close()
        # except:
...
def UserEdit(request,id):
    cur.execute("SELECT * FROM `user_master` WHERE `User_Id` = {}".format(id))
    data = cur.fetchone()
    return render(request, 'userside/updateacc.html', {'Mdata': data})

def UserEditProcess(request,id):
    if request.method == 'POST':
        uid=id
        print(request.POST)
        name = request.POST['username']
        gender = request.POST['gender']
        email = request.POST['useremil']
        address = request.POST['Address']
        mobile = request.POST['Mobile']
        cur.execute("update `user_master` set `User_Name` ='{}' where `User_Id`='{}'".format(name,uid))
        cur.execute("update `user_master` set `Gender` ='{}' where `User_Id`='{}'".format(gender,uid))
        cur.execute("update `user_master` set `Email` ='{}' where `User_Id`='{}'".format( email,uid))
        cur.execute("update `user_master` set `Address` ='{}' where `User_Id`='{}'".format(address,uid))
        cur.execute("update `user_master` set `Mobile` ='{}' where `User_Id`='{}'".format(mobile,uid))
        conn.commit()
